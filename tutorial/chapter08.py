...
LastEditors: Bowen
Description: 
FilePath: /fastapi-tutorial/tutorial/chapter08.py
'''
...
"""【见 run.py 】Middleware 中间件"""

# 注：带yield的依赖的退出部分的代码 和 后台任务 会在中间件之后运行


"""【见 run.py】CORS (Cross-Origin Resource Sharing) 跨源资源共享"""
...
def bg_task(framework: str):
    with open("ReadME2.md", mode='a') as f:
...
async def run_bg_task(framework: str, background_task: BackgroundTasks):
    '''
    @description: 
    @param {str} framework: 被调用的后台任务函数的参数
    @param {BackgroundTasks} background_task: FastAPI.BackgroundTasks
    @return {*}
    '''
